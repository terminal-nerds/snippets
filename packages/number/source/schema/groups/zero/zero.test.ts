...
describe("validateZero(value)", () => {
	it(throws(ZodError).on(`non-zero`).samples(SAMPLE_NON_ZEROES), () => {
...
describe("isZero(value)", () => {
	it(returns(false).on(`non-zero`).samples(SAMPLE_NON_ZEROES), () => {
		for (const nonZeroSample of SAMPLE_NON_ZEROES) {
			expect(isZero(nonZeroSample)).toBe(false);
		}
	});

	it(returns(true).on(`zero`).samples(SAMPLE_ZEROES), () => {

...
describe(`exclude(array, items)`, () => {
	...
	it(throws(ZodError).on(`passed keys that is not an array`), () => {
		for (const sample of SAMPLE_PRIMITIVES) {
			// @ts-expect-error Testing
			expect(() => exclude([1, 2], sample)).toThrowError(ZodError);
		}
	});
	...
	describe(
		...
		() => {
			const excluded = exclude(SAMPLE_ARRAY, itemsToExclude);
...
describe(`extract(array, items)`, () => {
	it(throws(ZodError).on(`passed array that is not an array`), () => {
		for (const sample of SAMPLE_PRIMITIVES) {
			// @ts-expect-error Testing
			expect(() => extract(sample, [1, 2])).toThrowError(ZodError);
		}
	});
	...
	it(throws(ZodError).on(`passed keys that is not an array`), () => {
		for (const sample of SAMPLE_PRIMITIVES) {
			// @ts-expect-error Testing
			expect(() => extract([1, 2], sample)).toThrowError(ZodError);
		}
	});
	...
	describe(
		...
		() => {
			const extracted = extract(SAMPLE_ARRAY, itemsToExtract);
